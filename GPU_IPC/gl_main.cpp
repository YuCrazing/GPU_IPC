...
void initScene1(int argc, char** argv)
{
    ...
    if(ipc.pcg_data.P_type)
    {
        ...
        CUDA_SAFE_CALL(cudaMemcpy(ipc.pcg_data.MP.d_neighborStart,
                                  ...
                                  ipc.vertexNum * sizeof(unsigned int),
                                  cudaMemcpyHostToDevice));
    ...
    if(ipc.pcg_data.P_type)
    {
        ...
        CUDA_SAFE_CALL(cudaMemcpy(tetMesh.surface.data(),
                                  ipc._faces,
                                  ipc.surface_Num * sizeof(uint3),
                                  cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(tetMesh.surfEdges.data(),
                                  ipc._edges,
                                  ipc.edge_Num * sizeof(uint2),
                                  cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaMemcpy(tetMesh.surfVerts.data(),
                                  ipc._surfVerts,
                                  ipc.surf_vertexNum * sizeof(uint32_t),
                                  cudaMemcpyDeviceToHost));
    }
