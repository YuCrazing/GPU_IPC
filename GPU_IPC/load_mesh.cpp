...
int tetrahedra_obj::getVertNeighbors()
{
    ...
    for(int i = 0; i < tetrahedraNum; i++)
    {
        ...
        if(SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[0], edgI[1]))
               == SFEdges_set.end()
           && SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[1], edgI[0]))
                  == SFEdges_set.end())
        {
            SFEdges_set.insert(pair<uint64_t, uint64_t>(edgI[0], edgI[1]));
        }
        if(SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[0], edgI[2]))
               == SFEdges_set.end()
           && SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[2], edgI[0]))
                  == SFEdges_set.end())
        {
            SFEdges_set.insert(pair<uint64_t, uint64_t>(edgI[0], edgI[2]));
        }
        if(SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[0], edgI[3]))
               == SFEdges_set.end()
           && SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[3], edgI[0]))
                  == SFEdges_set.end())
        {
            SFEdges_set.insert(pair<uint64_t, uint64_t>(edgI[0], edgI[3]));
        }
        if(SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[1], edgI[2]))
               == SFEdges_set.end()
           && SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[2], edgI[1]))
                  == SFEdges_set.end())
        {
            SFEdges_set.insert(pair<uint64_t, uint64_t>(edgI[1], edgI[2]));
        }
        if(SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[1], edgI[3]))
               == SFEdges_set.end()
           && SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[3], edgI[1]))
                  == SFEdges_set.end())
        {
            SFEdges_set.insert(pair<uint64_t, uint64_t>(edgI[1], edgI[3]));
        }
        if(SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[2], edgI[3]))
               == SFEdges_set.end()
           && SFEdges_set.find(pair<uint64_t, uint64_t>(edgI[3], edgI[2]))
                  == SFEdges_set.end())
        {
